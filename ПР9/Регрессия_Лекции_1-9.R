...
mydata$prob  <- predict(object = mylogit, type = "response") # предсказанные вероятности в переменной
pred_fit <- prediction(mydata$prob, mydata$admit) 
...
optim<-0.32 # подобрать значение
abline(v= optim, lwd = 2, lty = 'dotted', col = 'black') 

library(pROC) # ROC- кривая и AUC 
test_prob = predict(mylogit, newdata = mydata, type = "response")
test_roc = roc(mydata$admit ~ test_prob, plot = TRUE, print.auc = TRUE)

# разделить % ошибок 
nrow(subset(mydata,admit == 1&prob>=optim))/nrow(subset(mydata,admit == 1)) # правильно +
nrow(subset(mydata,admit == 0 & prob<optim))/nrow(subset(mydata,admit == 0)) # правильно -
ttt<-mydata$prob[mydata$admit==0]
length(ttt[ttt>=optim])/length(ttt)
length(ttt[ttt<optim])/length(ttt)
ttt1<-mydata$prob[mydata$admit==1]
length(ttt1[ttt1>=optim])/length(ttt1)
length(ttt1[ttt1<optim])/length(ttt1)

#### картинка предсказанная вероятность ~ go.home // 19 March 2024 
er<-c(ttt1,ttt)
er<-data.frame( p = er, 
                v = factor(c(rep(2,length(ttt1)), 
                             rep(1,length(ttt)))))
ggplot(er, aes(x = p, group = v))+
  geom_dotplot(aes(fill = v), dotsize = 0.6, alpha = .5)+
  geom_vline(xintercept = optim - 0.01, linewidth = 0.75, color = "blue")+
  scale_fill_manual(values = c("Red", "Green"), 
                    name = "студент", 
                    labels = c("не поступил", "поступил"))+
  theme_bw()

optim<-0.33

### Мультиномиальная логистическая регрессия ######################
load("ml.rd") # write (writing score — оценка); 
#ses (social economic status — социально-экономический статус, три уровня: низкий, средний и высокий) 
#влияют на выбор программы обучения (prog)
with(ml, table(ses, prog)) # Посмотреть структуру
require(foreign)
require(nnet) # здесь нужная нам функция multinom()

ml$prog2 <- relevel(ml$prog, ref = "academic") # зададим контрольную группу
